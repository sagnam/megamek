...
public class FixFacingCommand extends ServerCommand {
    ...
    public FixFacingCommand(Server server) {
        super(server, "fixFacing",
                "Fix facing of unit.  Usage: /fixFacing # # where the first is the" +
                        "entity id number and the second is the new facing [N,NE,NE,S,SW,NW]");
    }
    ...
    public void run(int connId, String[] args) {
        try {
            ...
            if (ent.getOwner().getId() != connId) {
                ...
                server.sendServerChat(connId, "Facing must be one of [N,NE,NE,S,SW,NW]");
            } else {
